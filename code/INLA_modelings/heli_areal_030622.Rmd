...
# Observations  
```{r}
Heli.df = read.csv("data/processed/global_helicoverpa_data.csv", 
                  header = TRUE, sep=",")
...
range(Heli.df$Number.of.yearly.traps) #sampling effort

range(Heli.df$mean.count)
...
Region.counts = as.data.frame(
                      Heli.pnts@data %>%
                        ...
                        summarise(Moths = sum(mean.count),
                                  Traps = sum(Number.of.yearly.traps)))
head(Region.counts)
```
...
ggplot(hex_df, 
        aes(long,lat, group=group, fill = Traps)) + 
...
Model.hex$Traps[is.na(Model.hex$Traps)] = 0 #same for traps
...
J = inla.read.graph("J") #coverting to a matrix
...
### Fake Predictor
Creating a fake "temperature" variable as an example.
```{r}
Model.hex$Temperature = rnorm(dim(Model.hex)[1])
range(Model.hex$Temperature)
```
...
My.lst =  list(list(intercept1 = rep(1, dim(Model.hex)[1])), #Repeat value of 1 for each data point
          list(Temperature = Model.hex[,"Temperature"], #Fake "fixed" effect
               Effort = Model.hex[,"Traps"], # number of traps in hexagon, "Effort"
...
range(Model.hex$Moths.cut) #Updated range

Stack.1 = inla.stack(data = list(Y = Model.hex$Moths.cut),  #Response variable, Moth Counts w/ added cut point
...
Frm.0 = Y ~ -1 + intercept1 +      #Y is response variable, -1 tells software we're using a custom intercept1
                       f(Region1,  #Index of Region IDs
                        ...
                        f(ID.Region.Yr1,  #interaction effect
                          ...
                          constr=TRUE) +
                       Effort + Temperature #Fixed effects
...
Model.0 = inla(Frm.0, #formula above
      				 data = inla.stack.data(Stack.1), #data to use
...
Results summary
```{r}
summary(Model.0) #mean result values, specific values are plotted below 
```
...
cor(plot.fitted$Moths.cut, plot.fitted$Fitted) #rounder version with maximum cutoff
cor(plot.fitted$Moths, plot.fitted$Fitted) #original values before rounding, cutoff, etc..


ggplot(plot.fitted, aes(Moths.cut, Fitted)) +
