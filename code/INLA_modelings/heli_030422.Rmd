...
# Observations
```{r}
Heli.df = read.csv("global_helicoverpa_data.csv", 
                  header = TRUE, sep=",")
...
#Create fake covariate as an example
Model.in$Fake.predictor = rnorm(length(Model.in$Longitude))
range(Model.in$Fake.predictor)
...
Model.in$Effort = Model.in$Number.of.yearly.traps
...
My.lst = list(c(Field.1,  #Spatial Index
                ...
                list(Temperature = Model.in[,"Fake.predictor"], #Random variable, temperature for example
...
Model.in$Count.round = round(Model.in$mean.count, 0) #Response variable, rounded to integer
...
Frm.1 = Y ~ -1 + intercept.1 + # the -1 species we use a custom intercept (fixed effect)
                ...
                f(Species,   # iid random effect for spp differences (like a factor)
        				   ...
        				   model="iid") +
                Temperature + Effort #fixed effects
