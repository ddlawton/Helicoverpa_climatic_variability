...
## Model 2 with replicate in walk
adding a random walk for each climatic variable for each species


```{r}
#Model 2

unique(factor(Model.in$species))
unique(factor(Model.in2$Spp_rep))

Model.in2 <- Model.in %>%
  mutate(Spp_replicate = as.integer(factor(species))) # 1 = H. arm, 2 = H punc., 3 = H zea


Model.in2$range.air <- inla.group(Model.in2$Air_temp_mean , n = 50, method = "quantile")
Model.in2$range.precip <- inla.group(Model.in2$precip_mean, n = 50, method = "quantile")



My.lst2 = list(c(Field.1,  #Spatial Index
                list(intercept.1 = 1)), #Intercept
                list(Temperature = Model.in2[,"range.air"],
                     Precip = Model.in2[,"range.precip"],
                     Effort = Model.in2[,"effort"],   #Number of samples
                     Year = Model.in2[,"year.int"],   #Index for year order
                     Species = Model.in2[,"species"], #Species variable
                     Spp_replicate = Model.in2[,"Spp_replicate"],
                     Trap = Model.in2[,"trap"]))  #Variability by trap location
...
Stack.2 = inla.stack(data = list(Y = Model.in$Count.round), #Response variable  
                                  A = list(A.matrix, 1), #spatial matrix    
                            effects = My.lst2, #all variables of interest
                                tag = "moth.1") #arbitary label/name
...
Frm.2 = Y ~ -1 + intercept.1 + # the -1 species we use a custom intercept (fixed effect)
                ...
                f(Year,         #random effect for year
                   model="ar1", #autocorrelation between years
                   constr=TRUE) + 
                f(Spp_replicate,   # iid random effect for spp differences (like a factor)
        				   constr=TRUE, #center on zero
        				   model="iid")+
                f(Temperature,     #Year index
                   model="rw1",
                  replicate = Spp_replicate,# model as a random walk, smooth trend
                   constr=TRUE) + 
                f(Precip,     #Year index
                   model="rw1",
                  replicate = Spp_replicate, # model as a random walk, smooth trend
...
#Execute/run model
Model.2 = inla(Frm.2, 
               data = inla.stack.data(Stack.2, spde=spde.1), 
               family = "poisson", 
               verbose = TRUE,
               control.predictor = list(
                                      A = inla.stack.A(Stack.2), 
                                      compute = TRUE, 
                                      link = 1), 
               control.mode = list(restart = TRUE), #mean estimate from first run, speeds up re-runs
               control.compute=list(dic = TRUE, waic = TRUE)) 
...
My.lst3 = list(c(Field.1,  #Spatial Index
                list(intercept.1 = 1)), #Intercept
                list(Temperature = Model.in2[,"range.air"],
                     Precip = Model.in2[,"range.precip"],
                     Effort = Model.in2[,"effort"],   #Number of samples
...
Stack.3 = inla.stack(data = list(Y = Model.in$Count.round), #Response variable  
                                  A = list(A.matrix, 1), #spatial matrix    
                            effects = My.lst2, #all variables of interest
                                tag = "moth.1") #arbitary label/name
...
#Execute/run model
Model.3 = inla(Frm.3, 
               data = inla.stack.data(Stack.3, spde=spde.1), 
               family = "poisson", 
               verbose = TRUE,
               control.predictor = list(
                                      A = inla.stack.A(Stack.3), 
                                      compute = TRUE, 
                                      link = 1), 
               control.mode = list(restart = TRUE), #mean estimate from first run, speeds up re-runs
               control.compute=list(dic = TRUE, waic = TRUE)) 
...
#Model 4
this time switching replicate for group in the rw1
...
Model.in2 <- Model.in %>%
  mutate(Spp_replicate_re = as.integer(factor(species)),
         Spp_replicate_temp = as.integer(factor(species)),
         Spp_replicate_precip = as.integer(factor(species))) # 1 = H. arm, 2 = H punc., 3 = H zea
...
My.lst3 = list(c(Field.1,  #Spatial Index
                ...
                list(Temperature = Model.in2[,"range.air"],
                     ...
                     Year = Model.in2[,"year.int"],   #Index for year order
                     Species = Model.in2[,"species"], #Species variable
                     Spp_replicate_re = Model.in2[,"Spp_replicate_re"],
                     Spp_replicate_temp = Model.in2[,"Spp_replicate_temp"],
                     Spp_replicate_precip = Model.in2[,"Spp_replicate_precip"],
                     Trap = Model.in2[,"trap"]))  #Variability by trap location
...
Stack.3 = inla.stack(data = list(Y = Model.in$Count.round), #Response variable  
                                  A = list(A.matrix, 1), #spatial matrix    
                            effects = My.lst3, #all variables of interest
                                tag = "moth.1") #arbitary label/name


#Model formula

Frm.4 = Y ~ -1 + intercept.1 + # the -1 species we use a custom intercept (fixed effect)
                f(Field.1,   #spatial index
                   model=spde.1) + #spatial prior (random effect)
                f(Year,         #random effect for year
                   model="ar1", #autocorrelation between years
                   constr=TRUE) + 
                f(Spp_replicate_re,   # iid random effect for spp differences (like a factor)
        				   constr=TRUE, #center on zero
        				   model="iid")+
                f(Trap,   # iid random effect for spp differences (like a factor)
        				   constr=TRUE, #center on zero
                ...
                f(Temperature,     #Year index
                   model="rw1",
                  group = Spp_replicate_temp,# model as a random walk, smooth trend
                   constr=TRUE) + 
                f(Precip,     #Year index
                   model="rw1",
                  group = Spp_replicate_precip, # model as a random walk, smooth trend
                   constr=TRUE)
...
Model.in3 <- predict(Model.2,Model.in2)

Model.2$summary.random
...
```{r}
library(inlabru)
pred_mod <- predict(Model.2,data=Model.in2)

Model.4$summary.random$Spp_replicate_re
trend.df = Model.4$summary.random$Temperature[,c(1,2,4,6)]   #pull estimates from model
...
trend.df = Model.4$summary.random$Precip[,c(1,2,4,6)]   #pull estimates from model
